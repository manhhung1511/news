<?php 
 ...
 $title = 'Tìm kiếm cơ sở y tế - Songxanh24h.vn';
 $this->title = $title;
                        ...
                        <?php foreach ($results as $item) : ?>
                            <li class="content_search-item">
                                <article>
                                    ...
                                    <h3 style="font-size: 20px; font-weight: 500; line-height: 26px;"><a href="<?= Yii::$app->urlManager->createAbsoluteUrl(['crawl/hospital-detail', 'slug' => $item['slug'],'slug-category' => $item['slug_category']]) ?>"><?= $item['name'] ?></a></h3>
                                    <div class="post-meta-data"><span>
                                            Địa chỉ: <a href="" title=""><?= $item['address'] ?></a></span></div>
                                    <div class="body">
                                        <?= $item['description'] ?>
                                    </div>
                                </article>
...
$(document).ready(function() {
    ...
    if(currentUrl.includes('co-so-y-te')) {
        let fourthElement = document.querySelectorAll(".search_category-list li a")[3];
